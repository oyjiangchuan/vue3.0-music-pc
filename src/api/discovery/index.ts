...
import { convertBanner, convertRecommendList } from "./translate";
...
export const getRecommendList = (params: any) => {
  return request
    ...
    .then(convertRecommendList);
};

// 获取最新音乐数据
export const getNewSongs = () => {
  return request.get(REQUEST_URL.newSongs);
};

// 获取推荐mv数据
export const getPersonalizedMv = () => {
  return request.get(REQUEST_URL.mvs);
};
