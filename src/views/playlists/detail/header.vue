...
export default defineComponent({
  ...
  setup(props) {
    ...
    const playAll = () => {
      store.dispatch("music/startSong", props.songs[0]);
      store.commit(`music/${musicTypes.SET_PLAY_LIST}`, props.songs);
    };
    const tagsText = computed(() => {
      return Array.isArray(props.playlist.tags)
        ? props.playlist.tags.join("/")
        : "";
    });
    return {
      playAll,
